...
expect(!str_contains($row->data, 'data:image/png'), 'no duplicate signature image persisted');
...
expect(EE_Store::revise($id, $edit, $revision) === true, 'administrator can revise without reusing signature');
...
expect(!isset($correction['signature']) && !isset($correction['confirmed']), 'correction does not claim original signature or confirmation');
$html = EE_PDF::html($correction, $brand, $revisedRow->reference);
expect(str_contains($html, 'nicht erneut unterschrieben') && !str_contains($html, '<img class="signature"'), 'correction PDF clearly labelled and unsigned');
