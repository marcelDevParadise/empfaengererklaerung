...
final class EE_Admin {
    ...
    public static function settings(): void {
        ...
        <tr><th><label for="ee-<?= esc_attr($key) ?>-body">Nachricht</label></th><td><textarea class="large-text" rows="7" id="ee-<?= esc_attr($key) ?>-body" name="ee_settings[<?= esc_attr($key) ?>_body]" maxlength="5000" required><?= esc_textarea($s[$key . '_body']) ?></textarea><p class="description">Platzhalter: <code>{vorgang}</code>, <code>{name}</code>, <code>{vorname}</code>, <code>{nachname}</code>, <code>{email}</code>, <code>{telefon}</code>, <code>{anschrift}</code>, <code>{sendungsnummer}</code>, <code>{versanddienstleister}</code>, <code>{absender}</code>, <code>{empfaenger}</code>, <code>{sendungsinhalt}</code>, <code>{zustellempfaenger}</code>, <code>{zustelldatum}</code>, <code>{empfangsdatum}</code>, <code>{ort}</code>, <code>{datum}</code>, <code>{unternehmen}</code>. Reiner Text, kein HTML.</p></td></tr>
        <?php endforeach; ?>
        </tbody></table>
    ...
    private static function detail(int $id): void {
        ...
        <?php endforeach; ?></dl><h2><?= isset($original['_correction']) ? 'Überarbeitete Erklärung' : 'Unterschriebene Erklärung' ?></h2><p><?= esc_html(EE_Validation::statement($data)) ?></p><p><?= isset($original['_correction']) ? (!empty($original['_signature']) ? 'Die Originalunterschrift wurde übernommen. Die Änderungen wurden nicht erneut bestätigt.' : 'Die Unterschrift ist im Original-PDF enthalten; für diese ältere Erklärung ist sie nicht separat gespeichert.') : 'Richtigkeit und Vollständigkeit wurden bestätigt. Die Unterschrift ist im gespeicherten PDF enthalten.' ?></p></div>
